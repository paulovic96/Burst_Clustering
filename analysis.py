        ...
        for k in k_conditions:
            for reg in reg_conditions:
                if training:
                    ...
                    labels, eigvec, eigval = spectral_clustering(data, "euclidean", n_clusters, k=k, mutual=False,weighting="distance", normalize=True, reg_lambda=reg,save_laplacian=False, save_eigenvalues_and_vectors=False)
...
def main():
    ...
    training_set, training_set_indices, training_set_conditions = training_set_split.get_training_set_for_ambiguous_data(data, cluster_dict, ambiguous_conditions,ambiguous_tau = True, ambiguous_amplitude = True, proportion=0.5)
