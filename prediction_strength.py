...
def calculate_prediction_strength(labels_fitted, labels_centroid, unique_labels):
    ...
    A_valid_indices = [np.where(labels_fitted == i)[0] for i in
                       unique_labels]  # get indices for each clusters in validation data
    ...
    co_membership_matrix_centroids = get_co_membership_matrix(
        labels_centroid)  # compute co-membership for validation data labeld by nearest centroid
    ...
    for c in unique_labels:
        ...
        if n_valid_clusters[c] == 0 or n_valid_clusters[c] - 1 == 0:  # no or only one burst in class
            ...
            prediction_sterngth_c = 1 / (n_valid_clusters[c] * (n_valid_clusters[
                                                                    c] - 1)) * co_membership_sum  # prediction strength for cluster_i weighted by cluster size
...
def cross_valdation_prediction_strength(data, train_folds, valid_folds, train_fold_labels, valid_fold_labels,
                                        n_clusters=range(20), per_sample=False,
                                        valid_fold_labels_predicted_precomputed=None):
